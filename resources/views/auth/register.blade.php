...
<head>
    ...
    <title>{{__('site.title_register')}}</title>
</head>
<body>
...
<section class="login-content">
    <div class="logo">
        <h1>{{__('site.title_register')}}</h1>
    </div>
    <div class="login-box">
        <form class="login-form"  method="POST" action="{{ route('register') }}">
            @csrf
            <h3 class="login-head"><i class="fa fa-lg fa-fw fa-user"></i>Register IN</h3>

            <!-- Name -->
            <div class="form-group">
                <label class="control-label">{{ __('Name') }}</label>
                <input class="form-control form-control @error('name') is-invalid @enderror" type="text" name="name" value="{{ old('name') }}" autocomplete="name"  placeholder="name" autofocus>
                @error('name')
            ...
            <div class="form-group btn-container">
                ...
                <hr>
                <label class="control-label">If you have a account go for login page</label>
                <a href="{{route('login')}}" class="btn btn-primary btn-block"><i class="fa fa-sign-in fa-lg fa-fw"></i>SIGN IN</a>
